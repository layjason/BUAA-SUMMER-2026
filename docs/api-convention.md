...
- Version: 11 20260701 134718
  - 新增通用媒体文件下载端点 GET /common/media/{mediaId}，返回原始二进制流；上传接口返回的 MediaFile.url 现由服务端自动填充为该下载地址
...
- 上传成功后返回的 `MediaFile` 中 `url` 字段由服务端自动填充为 `/common/media/{mediaId}`，前端可直接将该 URL 用于 `<img src>` 等展示场景。
- 通用文件下载端点 `GET /common/media/{mediaId}` 返回原始二进制流，Content-Type 根据上传时的 MIME 类型设置，需登录后访问。
