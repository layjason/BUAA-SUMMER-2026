...
- Version: 3 20260703 025514
  - 更新文档：明确签名 URL 为长期地址，移除与过期/刷新相关的遗留描述
...
客户端应使用业务查询响应中的 `signedUrl` 展示媒体。`signedUrl` 为长期地址，但权限变更或软删除导致 `accessVersion` 递增后旧 URL 立即失效，此时客户端应重新查询业务接口获取新的 `signedUrl`。
