...
通过 `--scale` 可启动多个容器实例，共享同一 GPU。每增加一个实例额外占用约 1-2GB 显存（各实例独立加载模型，不共享权重）。根据 GPU 显存调整实例数量：

| GPU 显存 | 建议 `--scale` 值 |
|----------|-------------------|
| 8GB（RTX 3070） | 2-3 |
| 12GB（RTX 3080/4070） | 3-4 |
| 16GB（RTX 4080） | 5 |
| 24GB（RTX 3090/4090） | 5-10 |

启动后可通过 `nvidia-smi` 监控显存占用，按需调整。
