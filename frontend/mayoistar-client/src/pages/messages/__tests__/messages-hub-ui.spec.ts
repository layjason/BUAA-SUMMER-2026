...
describe('消息页 UI', () => {
  it('应实现会话搜索并移除列表相机图标', () => {
    ...
    expect(indexSource).toContain('searchableUsers')
    expect(indexSource).toContain('teamSearchResults')
    expect(indexSource).toContain('discoverTeams')
    expect(indexSource).toContain('collectUsersFromTeams')
    expect(indexSource).toContain('keywordMatchedTeams')
    expect(indexSource).toContain('refreshSearchCandidates')
