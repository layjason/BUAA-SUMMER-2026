 ...
 * 活动搜索集成测试。
 *
 * <p>类职责：验证活动搜索服务与 H2 测试数据库、JPA Specification、JSON 标签字段的真实协作。
 *
...
class ActivitySearchIntegrationTests {
     ...
     *
     * <p>前置条件：数据库存在活动标签 JSON 字段和公开活动数据。
     *
    ...
    void searchWorksAgainstH2Database() {
        PageResult<ActivityDtos.ActivitySummary> result = activitySearchService.search(new ActivitySearchCriteria(
                "桌游",
                List.of("桌游"),
                "北京",
    ...
    void mapPointsWorkAgainstH2Database() {
        List<ActivityDtos.ActivityMapPoint> points = activitySearchService.mapPoints(
                new ActivitySearchCriteria(null, null, "北京", null, null, null, null, 39.908, 116.397, 1000, 1, 20));
