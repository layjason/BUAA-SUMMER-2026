...
public class SubmitActivityValidator {
     ...
     *
     * <p>后置条件：所有必填字段通过校验时正常返回；任一字段不满足则抛出 BusinessException(20004) 或 BusinessException(20005)。
     *
     * <p>不变量：不修改传入的活动实体。
     *
     ...
     * <p>前置条件：activity 非空。
     *
     * <p>后置条件：reviewStatus 为 draft 或 changeRequired 时通过，否则抛出 20005。
     */
    private void validateReviewState(Activity activity) {
        switch (activity.getReviewStatus()) {
            ...
            case pending -> throw new BusinessException(20005, "当前活动已在审核中，不能重复提交");
            case approved -> throw new BusinessException(20005, "当前活动已审核通过，不能重复提交");
            case rejected -> throw new BusinessException(20005, "当前活动已被驳回，请创建新活动后重新提交");
        }
    }
     ...
     * <p>前置条件：activity 非空。
     *
     * <p>后置条件：startAt 与 endAt 均非空，且 endAt 严格晚于 startAt 时通过，否则抛出 20004。
     */
    private void validateSchedule(Activity activity) {
        Instant startAt = activity.getStartAt();
        Instant endAt = activity.getEndAt();
        if (startAt == null) {
            throw new BusinessException(20004, "活动开始时间不能为空");
        }
        if (endAt == null) {
            throw new BusinessException(20004, "活动结束时间不能为空");
        }
     ...
     * <p>前置条件：activity 非空。
     *
     * <p>后置条件：经纬度、城市、地址四者均非空时通过，否则抛出 20004。
     */
    private void validateLocation(Activity activity) {
        if (activity.getPointLon() == null
            ...
            throw new BusinessException(20004, "活动地点信息不完整，请提供地图选点和城市、详细地址");
        }
    }
     ...
     * <p>前置条件：activity 非空。
     *
     * <p>后置条件：capacity 非空且大于 0 时通过，否则抛出 20004。
     */
     ...
     *
     * <p>后置条件：registrationDeadline 非空且不晚于活动开始时间时通过，否则抛出 20004。
     */
    private void validateRegistrationDeadline(Activity activity) {
        ...
        if (registrationDeadline == null) {
            throw new BusinessException(20004, "报名截止时间不能为空");
        }
     ...
     * <p>前置条件：activity 非空。
     *
     * <p>后置条件：feeAmount 为非负数或无费用时通过，否则抛出 20004。
     */
    private void validateFee(Activity activity) {
        if (activity.getFeeAmount() != null && activity.getFeeAmount().compareTo(java.math.BigDecimal.ZERO) < 0) {
            throw new BusinessException(20004, "活动费用不能为负数");
        }
    }
