 ...
 *
 * <p>类职责：调用配置中的大模型接口，提取第一条 assistant 消息内容。
 *
...
public class OpenAiCompatibleActivityPlanningClient implements ActivityPlanningClient {
     ...
     *
     * <p>后置条件：成功时返回 assistant content；失败时返回可展示错误，不向上抛出供应商异常。
     *
    ...
    public ActivityPlanningClientResult generate(String prompt) {
        if (!configured()) {
            return ActivityPlanningClientResult.failed("AI 活动策划接口尚未配置");
        }
        ...
        try {
            ...
            if (content == null || content.isBlank()) {
                ...
                return ActivityPlanningClientResult.failed("AI 活动策划暂时不可用，请稍后重试");
            }
            return ActivityPlanningClientResult.succeeded(content);
        } catch (RestClientException exception) {
            log.warn("AI 活动策划 HTTP 调用失败，endpointConfigured=true");
            return ActivityPlanningClientResult.failed("AI 活动策划服务暂时不可用，请稍后重试");
