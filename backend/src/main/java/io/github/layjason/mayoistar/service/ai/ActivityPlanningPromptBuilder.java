 ...
 * AI 活动策划提示词构造器。
 *
 * <p>类职责：将活动策划请求转换为稳定的中文提示词，并约束模型返回可解析 JSON。
 *
...
public class ActivityPlanningPromptBuilder {
     ...
     *
     * <p>后置条件：返回包含用户输入、字段约束和输出格式要求的提示词。
     *
     * <p>不变量：不修改 request，不读取外部配置。
     *
    ...
    public String build(AiDtos.ActivityPlanningRequest request) {
        return String.join(
                ...
                "- 期望参与人数：" + expectedParticipants(request.getExpectedParticipants()),
    ...
    private String expectedParticipants(Integer expectedParticipants) {
        return expectedParticipants == null ? "未指定" : expectedParticipants.toString();
    }
