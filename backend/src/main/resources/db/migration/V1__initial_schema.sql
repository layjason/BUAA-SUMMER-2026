...
COMMENT ON COLUMN admins.login_attempts IS '连续登录失败次数，默认 0。后置条件：登录成功后重置为 0';
COMMENT ON COLUMN admins.last_failed_login_at IS '最后一次登录失败的时间，UTC 时区。用于计算锁定窗口';
COMMENT ON COLUMN admins.locked_until IS '账号锁定截止时间，UTC 时区。null 表示未锁定。后置条件：超过此时间且不在冻结窗口内的登录尝试自动解除锁定';
