...
- JSON 接口统一返回 `APIResponse<Data>`；二进制导出接口可直接返回文件流。
...
4. 对请求封装层统一处理 `APIResponse<Data>`：
   - `code === 200` 时读取 `data`。
   - `code !== 200` 时展示 `message` 或转换为业务错误。
...
2. Controller 返回结构统一包装为 `APIResponse<Data>`，并保持 HTTP 状态码规则与 `docs/api-convention.md` 一致。
...
3. 网络层统一处理 `APIResponse<Data>`：
   - 成功业务响应读取 `data`。
   - 业务失败根据 `code` 和 `message` 显示错误状态。
...
- Web 和 Android 可基于 `openapi.yaml` 启动 Mock Server，用于后端接口完成前的页面开发。
